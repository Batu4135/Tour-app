...
function truncateTextToWidth(text: string, font: PDFFont, size: number, maxWidth: number): string {
...
function fitFontSizeToWidth(params: {
  ...
  const step = params.step ?? 0.4;
  let size = params.initialSize;

  while (
    size > params.minSize &&
    params.texts.some((text) => params.font.widthOfTextAtSize(text, size) > params.maxWidth)
  ) {
    size -= step;
  }

  return Math.max(params.minSize, size);
}
...
export function drawDraftVoucherPage(
  ...
  const left = s(isPrintLayout ? 24 : 56);
  const right = s(isPrintLayout ? 568 : 539);
  const qtyCenter = s(isPrintLayout ? 48 : 80);
  const qtyHeaderX = s(isPrintLayout ? 24 : 62);
  const skuX = options?.showSku ? s(isPrintLayout ? 86 : 112) : s(isPrintLayout ? 100 : 118);
  const productX = options?.showSku ? s(isPrintLayout ? 156 : 168) : s(isPrintLayout ? 108 : 118);
  const lineTotalRight = s(isPrintLayout ? 556 : 528);
  const productMaxWidth = lineTotalRight - productX - s(isPrintLayout ? 78 : 54);
  ...
  const rawCustomerName = String(draft.customer?.name ?? "").slice(0, 42);
  const customerAreaLeft = left + s(isPrintLayout ? 44 : 0);
  const customerAreaRight = lineTotalRight - s(isPrintLayout ? 124 : 0);
  const customerMaxWidth = customerAreaRight - customerAreaLeft;
  while (bold.widthOfTextAtSize(rawCustomerName, customerSize) > customerMaxWidth && customerSize > s(16)) {
    customerSize -= s(0.5);
  }
  const customerName = truncateTextToWidth(rawCustomerName, bold, customerSize, customerMaxWidth);
  const customerWidth = bold.widthOfTextAtSize(customerName, customerSize);
  page.drawText(customerName, {
    x: customerAreaLeft + (customerMaxWidth - customerWidth) / 2,
    y: customerY,
    size: customerSize,
    font: bold,
    color: textColor
  ...
  page.drawText("Menge", { x: qtyHeaderX, y: headerLabelY, size: headerLabelSize, font: bold, color: muted });
  if (options?.showSku) {
  ...
  const fullRowTexts = displayRows.map((row) =>
    row.type === "product"
      ? String(row.line.product?.name ?? "").trim()
      : `${licenseLabel(row.entry.licenseType)} ${weightKg(row.entry.totalWeightGrams)} kg`
  );
  const fittedRowFontSize = fitFontSizeToWidth({
    texts: fullRowTexts,
    font: regular,
    maxWidth: productMaxWidth,
    initialSize: rowFontSize,
    minSize: s(isPrintLayout ? 9.5 : 7),
    step: s(0.35)
  });
  const skuFontSize = Math.max(s(6.4), Math.min(fittedRowFontSize * 0.9, s(isPrintLayout ? 14 : 9)));
  const qtyFontSize = Math.max(
    s(5.5),
    Math.min(isPrintLayout ? s(14.5) : s(12.5), Math.max(fittedRowFontSize * 0.72, rowHeight * 0.4))
  );
  const qtyBadgeHeight = Math.max(s(4.5), Math.min(isPrintLayout ? s(24) : s(16), rowHeight * 0.58));
  ...
  displayRows.forEach((row, index) => {
    ...
    const primaryY = rowCenterY - fittedRowFontSize * 0.3;
    const qtyText = row.type === "product" ? formatQuantity(row.line.quantity) : "1";
    const sku = row.type === "product" ? String(row.line.product?.sku ?? "").trim() : row.entry.licenseType;
    const rawName =
      row.type === "product"
        ? String(row.line.product?.name ?? "").trim()
        : `${licenseLabel(row.entry.licenseType)} ${weightKg(row.entry.totalWeightGrams)} kg`;
    const name =
      regular.widthOfTextAtSize(rawName, fittedRowFontSize) <= productMaxWidth
        ? rawName
        : truncateTextToWidth(rawName, regular, fittedRowFontSize, productMaxWidth);
    const lineTotal =
      row.type === "product"
    ...
    page.drawText(name, { x: productX, y: primaryY, size: fittedRowFontSize, font: regular, color: textColor });
  ...
  const summaryLabelSize = Math.min(s(isPrintLayout ? 13.2 : 10), Math.max(s(8.5), fittedRowFontSize * 0.9)) * summaryScale;
  const summaryValueSize = Math.min(s(isPrintLayout ? 14.2 : 12), Math.max(s(9.5), fittedRowFontSize * 0.98)) * summaryScale;
  const summaryTotalSize = Math.min(s(isPrintLayout ? 16.5 : 12), Math.max(s(10.5), fittedRowFontSize * 1.02)) * summaryScale;
  ...
  const summaryRightX = s(isPrintLayout ? 326 : 370);
  ...
  page.drawLine({
    start: { x: s(isPrintLayout ? 332 : 372), y: ruleY },
    end: { x: lineTotalRight, y: ruleY },
    thickness: s(2),
    color: accent
  });

  page.drawText(paymentMethodText(draft.paymentMethod), {
    x: left,
    y: totalValueY,
  ...
  page.drawText("Gesamt", { x: s(isPrintLayout ? 340 : 384), y: totalLabelY, size: summaryLabelSize, font: regular, color: muted });
  drawRightText({
    page,
