...
export function drawDraftVoucherPage(
  ...
  const headerY = s(676, scale);
  page.drawLine({
    start: { x: left, y: s(696, scale) },
    end: { x: right, y: s(696, scale) },
    color: soft,
    thickness: s(1, scale)
  });
  ...
  page.drawLine({
    start: { x: left, y: s(664, scale) },
    end: { x: right, y: s(664, scale) },
    color: soft,
    thickness: s(1, scale)
  });

  const rowStartY = s(640, scale);
